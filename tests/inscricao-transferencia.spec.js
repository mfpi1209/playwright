...
test('test', async ({ page }) => {
  ...
  console.log(`   🔍 Buscando forma de ingresso com: "${textoBuscaIngresso}" para encontrar: "${CLIENTE.tipoIngresso}"`);
  ...
  if (erroVisivel) {
    ...
    console.log(`❌  Tipo: "${ingressoUsado}"`);
  ...
  if (linkProva) {
    ...
    if (ingressoUsado.toLowerCase() !== CLIENTE.tipoIngresso.toLowerCase()) {
      console.log(`📝 INGRESSO ALTERNATIVO UTILIZADO: "${ingressoUsado}"`);
      console.log(`   (Ingresso original solicitado: "${CLIENTE.tipoIngresso}")`);
    }
  } else {
